...
- `DescuentoGlobalPorc` o `PorcentajeDescuentoGlobal` (numero, 0-100). Opcional. Porcentaje de descuento a aplicar globalmente sobre el precio unitario de cada detalle. **No puede usarse junto con `DescuentoGlobal`.**
- `DescuentoGlobal` (numero, >=0). Opcional. Monto total del descuento global en la factura. Se distribuye proporcionalmente entre los detalles según su cantidad. **No puede usarse junto con `DescuentoGlobalPorc`.**
...
- `DescuentoGlobal` (numero, >=0). Opcional. Solo se especifica cuando el documento usa `DescuentoGlobal` (no `DescuentoGlobalPorc`). Se calcula automáticamente a partir del `DescuentoGlobal` del documento.
...
El app soporta descuentos a nivel de detalle y descuentos globales. **No se puede usar `DescuentoGlobalPorc` y `DescuentoGlobal` al mismo tiempo en el documento.**

1. **Descuento de detalle** (`Descuento`): se aplica directamente a cada `DetalleDocumento`. Siempre disponible.
2. **Descuento global por porcentaje** (`DescuentoGlobalPorc` o `PorcentajeDescuentoGlobal`): porcentaje aplicado al precio unitario de cada detalle. 
   - Cuando se usa: **NO especificar `DescuentoGlobal` en el documento ni en los detalles.**
3. **Descuento global por monto** (`DescuentoGlobal`): monto total de descuento que se distribuye proporcionalmente entre los detalles según su cantidad.
   - Cuando se usa: **NO especificar `DescuentoGlobalPorc` en el documento.**
...
- `tax_type` en 1..5.
- `global_discount_percentage` entre 0 y 100. No puede usarse junto con `global_discount`.
- `global_discount` >= 0. Monto total de descuento global (se distribuye entre los detalles). No puede usarse junto con `global_discount_percentage`.
...
- `quantity` > 0.
- `discount` >= 0.
- `global_discount` >= 0. Solo presente cuando el documento usa `global_discount` (no `global_discount_percentage`). Se calcula automáticamente.
...
### Factura con Descuento Global por Monto (XML)
...
### Factura con Descuento Global por Porcentaje (XML)
...
<DocumentoElectronico>
  <TipoDocumento>1</TipoDocumento>
  <NumeroDeDocumentoCompleto>001-001-0000103</NumeroDeDocumentoCompleto>
  <FechaDocumento>2026-02-13 11:15:00</FechaDocumento>
  <TipoTransaccion>1</TipoTransaccion>
  <TipoImpuesto>1</TipoImpuesto>
  <Total>180000.00</Total>
  <TipoOperacion>1</TipoOperacion>
  ...
  <Cliente>
    <RazonSocial>Cliente con Descuento Porcentaje S.A.</RazonSocial>
    <TipoContribuyente>2</TipoContribuyente>
    <Ruc>494829</Ruc>
    <Dv>7</Dv>
    <Correo>cliente@ejemplo.com</Correo>
    <Direccion>Av. Principal 789</Direccion>
    <NumeroCasa>789</NumeroCasa>
    <CodigoDepartamento>1</CodigoDepartamento>
    <CodigoDistrito>1</CodigoDistrito>
    <CodigoCiudad>1</CodigoCiudad>
  </Cliente>
...
```json
{
  "DocumentoElectronico": {
    "TipoDocumento": 1,
    "NumeroDocumentoCompleto": "001-001-0000103",
    "FechaDocumento": "2026-02-13 11:15:00",
    "TipoTransaccion": 1,
    "TipoImpuesto": 1,
    "Total": 180000.00,
    "TipoOperacion": 1,
    ...
    "Cliente": {
      "RazonSocial": "Cliente con Descuento Porcentaje S.A.",
      "TipoContribuyente": 2,
      "Ruc": "494829",
      "Dv": "7",
      "Correo": "cliente@ejemplo.com",
